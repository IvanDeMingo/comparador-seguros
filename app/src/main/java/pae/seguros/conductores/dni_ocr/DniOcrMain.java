package pae.seguros.conductores.dni_ocr;
...
import pae.seguros.conductores.UserForm;

public class DniOcrMain extends Activity {
...
class Pantalla extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        String mensaje = "Focus the 3 OCR character lines of the DNI";
        ...
        int edad = datosOCR.getEdad();
        char sexo = datosOCR.getSexo();
        if (dígito != '?' && confirmación >= CONFIRMACIÓN_ACEPTABLE && edad != 0 && sexo != '?') {
            if (tiempo == 0)
...
class DatosOCR {
    ...
    private static final int COLUMNA_SEXO = COLUMNA_FECHA_NACIMIENTO + TAMAÑO_FECHA + 1;
    private static final int TAMAÑO_SEXO = 1;
    ...
    private static final String LETRAS_SEXO = "MF";
    ...
    private String caracteresPosibles(int columna, int fila) {
        ...
        else if (fila == 1 && columna == COLUMNA_SEXO) return LETRAS_SEXO;
        return DÍGITOS;
    }
    ...
    private boolean sexoValido() {
        sexo = valorCarácter(COLUMNA_SEXO, FILA_FECHAS);
    ...
    private boolean datosLeídosCorrectamente() {
        return númeroDNIVálido() && fechaNacimientoVálida()
                && fechaCaducidadVálida() && sexoValido()
    ...
    public String getDNI() { return String.valueOf(númeroDNI)+String.valueOf(letraNúmeroDNI); }

    public String getDoB() {
        Calendar fecha = Calendar.getInstance();
        int año = fecha.get(Calendar.YEAR);
        String auxAño =  new StringBuilder().append(fechaNacimiento[0]).append(fechaNacimiento[1]).toString();
        if (Integer.parseInt(auxAño) <= 99 && Integer.parseInt(auxAño) > año-2000)
            auxAño = new StringBuilder().append(1).append(9).append(auxAño).toString();
        else
            auxAño = new StringBuilder().append(2).append(0).append(auxAño).toString();
        String dia = new StringBuilder().append(fechaNacimiento[4]).append(fechaNacimiento[5]).toString();
        String mes = new StringBuilder().append(fechaNacimiento[2]).append(fechaNacimiento[3]).toString();

        return dia + '/' + mes + '/' + auxAño;
    }
    ...
    public int getEdad() {
        ...
        if (fechaNacimiento[0] != '?') {
            ...
            if (Integer.parseInt(auxAño) <= 99 && Integer.parseInt(auxAño) > año-2000)
                ...
                auxAño = new StringBuilder().append(2).append(0).append(auxAño).toString();
            edad = año - Integer.parseInt(auxAño);
            if (mes == Integer.parseInt(new StringBuilder().append(fechaNacimiento[2]).append(fechaNacimiento[3]).toString()) &&
                    dia < Integer.parseInt(new StringBuilder().append(fechaNacimiento[4]).append(fechaNacimiento[5]).toString()))
                --edad;
            else if (mes < Integer.parseInt(new StringBuilder().append(fechaNacimiento[2]).append(fechaNacimiento[3]).toString()))
                --edad;
        }
        return edad;
    }
